...
class Tremaine_Property {
	...
	protected $price_rental = '';
	protected $price_type = '';
	protected $price_rental_sqft;
	protected $beds = '';
	protected $bath_full = '';
	protected $bath_half = '';
	...
	public function get_price_rental() { return $this->price_rental; }
	public function get_price_type() { return $this->price_type; }
	public function get_price_rental_sqft() { return $this->price_rental_sqft; }
	...
	public function get_the_price( $rendered = true ){
		
		$price = 0;
		
		if ( $this->price_rental ){
			
			$price = $this->price_rental;
		...
		if ( $rendered ){
			...
			$price = money_format( '%.0n' , (int) $price );
			
		} // End if
		
		$types = explode(',', $this->price_type );
		
		if ( ! is_array( $types ) ) $types = array();
		
		if ( in_array( 'rent', $types ) && $price !== 'See Details' ){
	...
	public function set_property( $post ){
		...
		$this->type = get_post_meta( $this->get_id() , 'Property_Type' , true );
		$this->price_rental = get_post_meta( $this->get_id() , 'Price_Rental' , true );
		$this->price_type = get_post_meta( $this->get_id() , 'Price_Type' , true );
		$this->price_rental_sqft = get_post_meta( $this->get_id() , 'Price_Rental_Sqft' , true );
	...
	public function get_the_ppsf( $rendered = true ){
		
		if ( $this->price_rental_sqft ){
			
			$price = $this->price_rental_sqft;
