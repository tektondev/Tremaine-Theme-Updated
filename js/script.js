	...
	modal: {
		...
		show_frame:function( modal_content_wrap, width ){
			...
			if ( frame.length < 1 ) {
				...
				frame = tremaine.modal.add_frame_dynamic();
				
				frame.find('.tre-modal-window-content').html( modal_content_wrap.html() )
		...
		add_frame_dynamic:function(){
			
			if ( jQuery( '.tre-modal-frame.dynamic-modal').length < 1  ){
			...
			return jQuery( '.tre-modal-frame.dynamic-modal').find('.tre-modal-window');
